...
- [1. Включаем Hibernate](#1-Включаем-hibernate)
  ...
  - [Что выбрать? Вариант 1 (ядро Linux) или Вариант 2 (утилита uswsusp)?](#Что-выбрать-Вариант-1-ядро-linux-или-Вариант-2-утилита-uswsusp)
  - [2. (Вариант 1), Настройка Hibernation (спящего режима) из ядра Linux](#2-Вариант-1-Настройка-hibernation-спящего-режима-из-ядра-linux)
    - [1. Указываем где `/swapfile` с которого загружаться](#1-Указываем-где-swapfile-с-которого-загружаться)
    - [2. Устанавливаем размер образа системы и отключаем сжатие](#2-Устанавливаем-размер-образа-системы-и-отключаем-сжатие)
  - [2. (Вариант 2), Настройка Hibernation (спящего режима) через утилиту uswsusp](#2-Вариант-2-Настройка-hibernation-спящего-режима-через-утилиту-uswsusp)
    ...
    - [4. Заменяем стандартную функцию hibernation утилитой uswsusp](#4-Заменяем-стандартную-функцию-hibernation-утилитой-uswsusp)
...
## Что выбрать? Вариант 1 (ядро Linux) или Вариант 2 (утилита uswsusp)?

Выбирайте **ядро Linux** (**Вариант 1**), если с ней будут проблемы удалите все изменения, выполните команды `sudo update-initramfs -u` и `sudo update-grub` чтобы их применить и перезагрузите систему. После этого пробуйте **Вариант 2**.

Учтите, что лично я замечал некоторые странные ошибки при пробуждении с **uswsusp** (**Вариант 2**). Поэтому условно более надежным можно считать **Вариант 1**.

Если будете откатывать **uswsusp** (**Вариант 2**), чтобы попробоват **ядро Linux** (**Вариант 1**), то советую удалить **uswsusp** командой `sudo apt purge uswsusp` и перезагрузиться. Простого удаления настроек мне не хватало, если не удалить **uswsusp** система долго просыпается из hibernate когда переходите на **ядро Linux** (**Вариант 1**). Скорее всего при установке **uswsusp** меняет еще какие-то настройки, которые я не нашел.

## 2. (Вариант 1), Настройка Hibernation (спящего режима) из ядра Linux

### 1. Указываем где `/swapfile` с которого загружаться
...
### 2. Устанавливаем размер образа системы и отключаем сжатие
...
### 4. Заменяем стандартную функцию hibernation утилитой uswsusp
...
2. Если система зависла нажать и держать `alt + print screen` и не отпуская нажать по очереди клавиши `R E U I S B`. Это хуже чем перезагрузка системы, но лучше чем кнопка выключения (потому что есть корректное отмантирование дисков и завершение работы).
3. Если будете откатывать **uswsusp** (**Вариант 2**), чтобы попробоват **ядро Linux** (**Вариант 1**), то советую удалить **uswsusp** командой `sudo apt purge uswsusp` и перезагрузиться. Простого удаления настроек мне не хватало, если не удалить **uswsusp** система долго просыпается из hibernate, скорее всего при установке **uswsusp** меняет еще какие-то настройки, которые я не нашел.