...
<br>
[Tab Mix Plus](https://addons.mozilla.org/ru/firefox/addon/tab-mix-plus) - **Осторожно! В Firefox 52 ESR это дополнение делает контекстное меню страницы (по правой кнопке мыши) не предсказуемым! Пункты меню быстро нажимаются и приводят к случайным нажатиям. Возможно этого можно избежать отключая связанные с этим функции.** управление поведением вкладок. Можно установить принудительный одноконный режим (всё только во вкладках). Функция перезагрузки вкладки по таймеру, переименование вкладок, горячие клавиши, прогресс загрузки для каждой вкладки отдельно, множество других функций.
<br>
