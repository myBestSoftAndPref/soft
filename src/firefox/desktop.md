...
**Заметка (временная).** Mozilla встроила запрет на запуск Firefox как sudo, теперь просто так обновить от sudo нельзя, нужно запускать Firefox от сессии пользователя root (инструкция будет добавлена позже) <br>
...
7. Чтобы обновить из меню "О программе" запускаем firefox через sudo:
    ...
    3. Запустите командой <sub>(где ```/opt/my_firefox/firefox``` это путь к вашему Firefox)</sub>:
