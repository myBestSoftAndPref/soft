...
<br>
**A8.1.** Формат ```mozlz4``` создан Mozilla на основе ```lz4``` формата.
...
2. Открыть блокнот нажав ```shift``` + ```F4``` и вводить команды, которые ниже. В меню блокнота выбрать: ```Environment``` > ```Browser``` (```Среда``` > ```Браузер```)
