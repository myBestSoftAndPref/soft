Минимальны набор дополнений, стилей и настроек для **release** версии  
...
**A2.4** На странице ```about:config``` 1) Установить ```browser.tabs.remote.autostart``` в ```true``` 2) Создать параметр типа ```Boolean``` с именем ```browser.tabs.remote.force-enable```, значение ```true``` 3) Перезапустить Firefox
...
**A3.** При загрузке Firefox передает серверу список языков вашей версии Firefox. На ее основе делается выбор версии языка сайта. Изменить язык можно на странице ```about:config```. Для русского языка на сайтах установить: ```intl.accept_languages``` в ```ru-RU, ru, en-US, en```. Для английского языка: ```en-US, en```
...
**>>Инструкция** (для опция на странице ```about:support```):  
0) Официальная статья о принудительном включении аппаратного ускорения: [Blocked_Graphics_Drivers](https://wiki.mozilla.org/Blocklisting/Blocked_Graphics_Drivers)  
```1) Официальные```  
