...
**A** Это рекомендуется пользователям Ubuntu. На данный момент обновления в Ubuntu могут задерживаться на несколько дней. Не устанавливайте Firefox из snap в Ubuntu, snap имеет проблемы с отображением курсора. **Заметка (временная).** Mozilla встроила запрет на запуск Firefox как sudo, теперь просто так обновить от sudo нельзя, нужно запускать Firefox от сессии пользователя root (инструкция будет добавлена позже) <br>
